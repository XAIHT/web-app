...
2. **Single source of truth**: a **git tag** of the form `v1.40.1`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.40.1/`)
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.40.1/       (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.40.1"
            ├── Console banner: "--- [VERSION] Tlamatini 1.40.1"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.40.1` | `1.40.1` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.40.1"; python build.py` | `1.40.1` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.40.1 -m "..."; python build.py` | always the bare base tag → `1.40.1` (distance / dirty state never appear in the version string) |
...
```powershell
git tag -a v1.40.1 -m "Release 1.40.1: <one-line summary>"
```
...
All three scripts will pick up `v1.40.1` automatically (precedence #3 — git describe finds an exact tag at HEAD).

You'll see this in each script's output:
```
Tlamatini version : 1.40.1
...
The final artefact is **`dist/Tlamatini_Release_v1.40.1/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.40.1`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.40.1`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.40.1`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.40.1","build":"1.40.1","commit":"abc1234","date":"2026-05-18T12:00:00Z","source":"generated"}
  ```
...
A safer flow: **never delete tags**. If you tagged `v1.20.0` at the wrong commit, bump to `v1.40.1` at the right commit and move on. SemVer was designed assuming you'd do exactly this.
...
TAG IT                git tag -a v1.40.1 -m "Release 1.40.1"
                      git push origin v1.40.1
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.40.1/
                      About dialog : "Tlamatini v1.40.1"
                      Exe → Properties → Details : ProductVersion = 1.40.1
                      curl /agent/version/ : {"version":"1.40.1", …}
                      Console banner : --- [VERSION] Tlamatini 1.40.1
...
UNDO A TAG            git tag -d v1.40.1
                      git push origin :refs/tags/v1.40.1
                      (… but prefer: just bump to v1.40.1 instead)
```
