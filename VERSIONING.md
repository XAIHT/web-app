...
2. **Single source of truth**: a **git tag** of the form `v1.48.17`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.48.17/`)
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.48.17/      (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.48.17"
            ├── Console banner: "--- [VERSION] Tlamatini 1.48.17"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.48.17` | `1.48.17` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.48.17"; python build.py` | `1.48.17` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.48.17 -m "..."; python build.py` | always the bare base tag → `1.48.17` (distance / dirty state never appear in the version string) |
...
```powershell
git tag -a v1.48.17 -m "Release 1.48.17: <one-line summary>"
```
...
All three scripts will pick up `v1.48.17` automatically (precedence #3 — git describe finds an exact tag at HEAD).

You'll see this in each script's output:
```
Tlamatini version : 1.48.17
...
The final artefact is **`dist/Tlamatini_Release_v1.48.17/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.48.17`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.48.17`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.48.17`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.48.17","build":"1.48.17","commit":"abc1234","date":"2026-08-16T12:00:00Z","source":"generated"}
  ```
...
A safer flow: **never delete tags**. If you tagged `v1.20.0` at the wrong commit, bump to `v1.48.17` at the right commit and move on. SemVer was designed assuming you'd do exactly this.
...
TAG IT                git tag -a v1.48.17 -m "Release 1.48.17"
                      git push origin v1.48.17
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.48.17/
                      About dialog : "Tlamatini v1.48.17"
                      Exe → Properties → Details : ProductVersion = 1.48.17
                      curl /agent/version/ : {"version":"1.48.17", …}
                      Console banner : --- [VERSION] Tlamatini 1.48.17
...
UNDO A BAD TAG        git tag -d v1.20.0
                      git push origin :refs/tags/v1.20.0
                      (... but prefer: publish the next valid patch instead)
```
