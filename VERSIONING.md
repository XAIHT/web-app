...
2. **Single source of truth**: a **git tag** of the form `v1.20.1`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.20.1/`)
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.20.1/       (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.20.1"
            ├── Console banner: "--- [VERSION] Tlamatini 1.20.1"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.20.1` | `1.20.1` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.20.1"; python build.py` | `1.20.1` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.20.1 -m "..."; python build.py` | always the bare base tag → `1.20.1` (distance / dirty state never appear in the version string) |
...
All three scripts will pick up `v1.20.1` automatically (precedence #3 — git describe finds an exact tag at HEAD).

You'll see this in each script's output:
```
Tlamatini version : 1.20.1
...
The final artefact is **`dist/Tlamatini_Release_v1.20.1/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.20.1`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.20.1`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.20.1`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.20.1","build":"1.20.1","commit":"abc1234","date":"2026-05-18T12:00:00Z","source":"generated"}
  ```
...
```powershell
git tag -d v1.20.1                    # delete local tag
git push origin :refs/tags/v1.20.1    # delete remote tag
git tag -a v1.20.1 -m "Release 1.20.1" # re-create at correct commit
git push origin v1.20.1
```

A safer flow: **never delete tags**. If you tagged `v1.20.1` at the wrong commit, bump to `v1.20.1` at the right commit and move on. SemVer was designed assuming you'd do exactly this.
...
TAG IT                git tag -a v1.20.1 -m "Release 1.20.1"
                      git push origin v1.20.1
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.20.1/
                      About dialog : "Tlamatini v1.20.1"
                      Exe → Properties → Details : ProductVersion = 1.20.1
                      curl /agent/version/ : {"version":"1.20.1", …}
                      Console banner : --- [VERSION] Tlamatini 1.20.1
...
UNDO A TAG            git tag -d v1.20.1
                      git push origin :refs/tags/v1.20.1
                      (… but prefer: just bump to v1.20.1 instead)
```
