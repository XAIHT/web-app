...
<p align="center">
  ...
  A self-hosted AI developer assistant that turns your PC into a workshop where 78 agents collaborate to actually <strong>get things done</strong>.
</p>
...
<p align="center">
  ...
  <a href="#95-agent-catalog-the-78-types-by-family"><img src="https://img.shields.io/badge/AGENTS-78-8A2BE2?style=for-the-badge&labelColor=2D2D2D" alt="78 Agents" /></a>
...
| 🔌 **External MCPs** | Connect to **any** Model Context Protocol server from a JSON file (the same `mcpServers` shape a Claude-Code `.mcp.json` uses) — no hardcoding. Four transports (stdio, streamable-HTTP, SSE, WebSocket), up to 5 active at once. Tick servers in the **External ▸ MCPs** menu and the LLM can call their tools in Multi-Turn. |
| 🎨 **Visual workflow designer** | Drag and drop 78 agent types, wire them together, run flows unattended. No code required to compose a pipeline. |
...
| 🔒 **Self-hosted** | The app, hybrid RAG, and all 78 agents run on your machine. The LLM is your choice — fully-local [Ollama](https://ollama.com/) models, Ollama Cloud (the shipped default), or cloud APIs like Anthropic Claude. With a cloud model, your prompts and code context are sent to that provider. |
...
- [3. Using the Chat (`/agent/`)](#3-using-the-chat-agent)
  ...
  - [3.20. Tutorial: the **Step-by-Step** toggle](#320-tutorial-the-step-by-step-toggle)
  - [3.21. The **External ▸ MCPs** menu — connect to any MCP server](#321-the-external--mcps-menu--connect-to-any-mcp-server)
  - [3.22. Tutorial: diagnose an external MCP before you connect (`chat_agent_mcp_doctor`)](#322-tutorial-diagnose-an-external-mcp-before-you-connect-chat_agent_mcp_doctor)
...
- [6. Unreal MCP — Driving Unreal Engine 5 from Tlamatini](#6-unreal-mcp--driving-unreal-engine-5-from-tlamatini)
  ...
  - [6.11. Blender MCP quick-start (the **Blenderer** agent)](#611-blender-mcp-quick-start-the-blenderer-agent)
...
- [9. Architecture at a Glance](#9-architecture-at-a-glance)
  ...
  - [9.5. Agent catalog (the 78 types, by family)](#95-agent-catalog-the-78-types-by-family)
...
**Tlamatini** (Nahuatl for *"one who knows"*) is a Django/Channels app you run on your own machine. It packages a hybrid RAG pipeline, a Multi-Turn tool-calling LLM loop, an ACPX runtime that spawns external coding-agent CLIs as child processes, an **Unreal MCP** client that drives Unreal Engine 5 and a **Blender MCP** client that drives Blender from chat or canvas, and a drag-and-drop workflow designer with 78 agent types — into one self-hosted install. The app and RAG run locally; the LLM backend is configurable: **Ollama** (local models *or* Ollama Cloud — the shipped default uses Ollama Cloud `:cloud` tags), **Anthropic Claude** (cloud), **Qwen vision** (Ollama).
...
```
┌───────────────────────────────────────────────────────────────────────────────┐
│ Tlamatini [Context ▼][Open in…▼][MCPs ▼][Tools ▼][Agents ▼][External ▼][Config ▼][DB ▼] │ ← top nav
├───────────────────────────────────────────────────────────────────────────────┤
│ Multi-Turn ☐ Exec Report ☐ ACPX ☐ Ask Execs ☐ Step-by-Step ☐ internet ☐  Clear ⌫ │ ← six toggles
├───────────────────────────────────────────────────────────────────────────────┤
│  ┌──── chat ────────────────┐   ┌──── code canvas ────────────────┐          │
...
The **six toolbar toggles** are independent (except **Ask Execs** and **Step-by-Step**, which only act while **Multi-Turn** is on). Tick whatever combination fits the task — each one is its own tutorial section below. The newest toggle, **Step-by-Step**, makes Tlamatini guide a hands-on setup *one concrete step at a time*, waiting for your confirmation before the next (see [§3.20](#320-tutorial-the-step-by-step-toggle)).
...
Newer still is the **External** dropdown (between **Agents** and **Config**): `External -> MCPs` opens the **External MCPs** catalog, where you tick up to 5 external Model Context Protocol servers to activate, search the list, or drag a `.json` file onto it to import new servers. Once a server is active, the LLM can call its tools in Multi-Turn. Full walkthrough in [§3.21](#321-the-external--mcps-menu--connect-to-any-mcp-server).
...
- Tlamatini now binds her **full Multi-Turn tool surface** when Multi-Turn is on — every core Python tool (`execute_command`, `agent_starter`, `googler`, the image-analysis pair, the `chat_agent_run_*` lifecycle helpers, …), every wrapped chat-agent tool, and every active external-MCP tool (`ext__<server>__<tool>`, see [§3.21](#321-the-external--mcps-menu--connect-to-any-mcp-server)). She no longer reasons over a narrowed planner subset, so she never lacks a needed tool mid-task. (The ACPX/Skill tools remain the one exception — they are bound only when the **ACPX** checkbox is also ticked, see [§3.7](#37-tutorial-the-acpx-toggle).)
...
| Symptom | Fix |
|---|---|
| LLM says "Tool X is not available" | With full-surface binding this usually means the tool is **disabled** in Configure MCPs/Tools/Agents, or it is an ACPX/Skill tool and the **ACPX** checkbox is off, or (for an external MCP) the server is not active in **External ▸ MCPs**. Enable it and retry. |
...
### 3.20. Tutorial: the **Step-by-Step** toggle

**Step-by-Step** is for hands-on setups where you want to do the work *with* Tlamatini, not have her run off and do it all at once. Like **Ask Execs**, it only acts while **Multi-Turn** is on (the checkbox stays disabled and greyed until you tick Multi-Turn). When unticked, Tlamatini behaves exactly like the normal Multi-Turn flow.

With **Step-by-Step ticked**, Tlamatini guides a procedure **one concrete step at a time**: she tells you exactly what to do (or runs exactly one action), then **waits for your confirmation** before moving to the next step. No autonomous tool-loop, no jumping ahead. It is the right toggle when you are wiring up a new external MCP, setting up a device, or walking through any multi-stage configuration where each step depends on something you do on your side first.

**Try this:** tick **Multi-Turn + Step-by-Step** and send:

> *"Help me connect the official `time` MCP server. Walk me through it step by step."*

Tlamatini gives you the first step only — and the exact thing to reply when it is done — then waits. Confirm, and she moves to the next. This pairs naturally with the **External ▸ MCPs** menu below.

### 3.21. The **External ▸ MCPs** menu — connect to any MCP server

Tlamatini is a **universal MCP client**: she can connect to and use the tools of **any** external Model Context Protocol server you declare in a JSON file — the same `mcpServers` shape a Claude-Code `.mcp.json` uses — with **no hardcoding**. This is separate from the two built-in *context* MCP services (System-Metrics / Files-Search, see [§9.2](#92-the-five-layers)) and separate from ACPX (which spawns coding-agent CLIs, [§5](#5-acpx--external-coding-agent-clis-as-tools)). It connects over **four transports**:

| Transport | When to use it | Example |
|---|---|---|
| **stdio** | A local server launched as a command. | a Docker `mcp/*` image, `npx -y @modelcontextprotocol/server-…`, `uvx …`, `python -m …` |
| **streamable-http** | An already-running server reachable over HTTP. | a hosted MCP endpoint |
| **SSE (legacy)** | An already-running server using the older Server-Sent-Events transport. | a legacy MCP endpoint |
| **WebSocket** | An already-running server reachable over `ws://`. | a local WebSocket MCP bridge |

Up to **5 servers** can be active at once. The **External ▸ MCPs** navbar dropdown (between **Agents** and **Config**) is the operator surface:

- A **searchable catalog** of every declared server, each with a checkbox.
- **Tick up to 5** to activate; click **Continue** to apply.
- **Drag a `.json` file** onto the dialog to import new servers (the `mcpServers` shape).

Once a server is active, its tools become available to the LLM in Multi-Turn, wrapped under the name `ext__<server>__<tool>`. The LLM also gets a set of **supervisor tools** to manage them itself: check **status**, **reconnect**, run **doctor** (diagnose), **list_tools**, **call** a tool, **import** a config, **set_active**, and **wait**. The catalog is stored in `external_mcps.json` and managed by `agent/external_mcp_manager.py`.

> **Verified live** driving 10 different no-key servers — `memory`, `sqlite`, `redis`, `fetch`, `time`, `everything`, `sequentialthinking`, `filesystem`, `git`, and `puppeteer` — each chaining the external server's tools with Tlamatini's own File-Creator.

HTTP surface: `GET`/`POST /agent/external_mcps/` (list / save the catalog), `POST /agent/external_mcps/activate/` (set the active ≤5), `POST /agent/external_mcps/import/` (import a `.json`). Frontend: `agent/static/agent/js/external_mcps_dialog.js` + `external_mcps_dialog.css`.

### 3.22. Tutorial: diagnose an external MCP before you connect (`chat_agent_mcp_doctor`)

Before a server connects cleanly, the prerequisites have to line up — the right runtime installed, the command on `PATH`, real (not placeholder) secrets. The **MCP Doctor** agent diagnoses a *catalogued* external MCP **without connecting to it** and tells you the exact next setup step.

Tick **only the Multi-Turn** checkbox (MCP Doctor is a normal Multi-Turn tool — not behind the ACPX/Skill surface). Then ask, for example:

> *"Run MCP Doctor on the `puppeteer` server and tell me what I still need to do to use it."*

Tlamatini calls **`chat_agent_mcp_doctor`**, which inspects the server's catalog entry and reports: its **transport**; the **runtime prerequisite** (Docker / Node / Python / …) and whether it is present; whether the **command is on `PATH`**; any **missing or placeholder secrets**; and the **exact next setup step**. It is read-only — it never starts the server. The result is captured as an `INI_SECTION_MCP_DOCTOR` block for the Exec Report and Parametrizer, and the visual counterpart is the **MCP Doctor** canvas node (see [§9.5](#95-agent-catalog-the-78-types-by-family)). Pair it with **Step-by-Step** ([§3.20](#320-tutorial-the-step-by-step-toggle)) to walk through fixing whatever it flags, one step at a time, then activate the server from the **External ▸ MCPs** menu above.
...
### 6.11. Blender MCP quick-start (the **Blenderer** agent)

Tlamatini's **second engine bridge** drives **Blender** through the *official* Blender MCP add-on (https://www.blender.org/lab/mcp-server/) — the same "drive an external creative app from chat or canvas" idea as Unreal MCP, but for Blender. One key difference shapes everything: where Unreal MCP dispatches named verbs, **the Blender MCP wire format is a *code-execution* protocol** — each request is `{"type":"execute","code":<python>,"strict_json":<bool>}` (NUL-framed) and Blender runs that Python (which assigns a `result` dict). Rather than make you hand-write Python every time, the **Blenderer** agent exposes a **rich action catalog** on top of that one primitive.

Because **Tlamatini speaks the add-on socket directly** (it does *not* run blender.org's `blmcp` stdio bridge and you do *not* need their bare chat client), the only things you install are **Blender + the add-on** — then the whole Tlamatini stack (canvas, Multi-Turn, Exec Report, Parametrizer pipelines, 76 sibling agents) sits on top.

**Get going in 3 steps:**

1. **Start the add-on.** In Blender: enable the Blender MCP add-on, turn on **Online access** in *Preferences → System*, and **start the server** in the add-on's preferences panel (defaults to `localhost:9876`). The agent does **not** launch Blender — it only connects to it.
2. **Drive it from chat** (tick **Multi-Turn**). Probe first, then build:
   - `Run Blender command with command='ping'` → expect `status: ok` and a `blender_version_string`.
   - `Run Blender command with command='create_object' and params.type='monkey' and params.name='Hero' and params.location=[0,0,2]`
   - `Run Blender command with command='set_material' and params.object_name='Hero' and params.color=[0.92,0.46,0.0]`
   - `Run Blender command with command='render'` → renders a still; with no `params.output_path` the `.png` lands under Tlamatini's **Temp** directory.
   - Try the catalog modal's **BLENDER FORGE** demo prompt for the whole sequence in one go.
3. **Or build a flow on the canvas.** Drag the **Blenderer** node in, set its `command` + `params` in the node dialog, and wire `Starter → Blenderer → Ender`. Chain several through **Parametrizer** to carry one step's `response_body` into the next (e.g. `create_object → Parametrizer → set_material → render`). A **Forker** after a Blenderer can branch on the section's `{status}`.

**Action catalog** (the `command` field):

| Kind | Commands |
|---|---|
| Passthrough | `execute_code` (run arbitrary `bpy` Python via `params.code` — the universal escape hatch; set a `result` dict to return data) |
| Read-only | `ping`, `scene_info`, `get_objects`, `get_object_detail` (`params.object_name`), `blendfile_summary` |
| Mutating / output | `create_object` (`params.type` cube/sphere/cylinder/cone/plane/monkey/torus, `params.name`, `params.location`), `delete_object` (`params.object_name`), `set_material` (`params.object_name`, `params.color`, `params.material`), `screenshot` (`params.output_path`), `render` (`params.output_path`) |

Every run emits an `INI_SECTION_BLENDERER` block (`host`, `port`, `command`, `status`, `error`, `response_body`) for Parametrizer/Forker, and **always** triggers `target_agents` (success or failure). Point at a remote Blender with `host=…` / `port=…`. If a command isn't in the catalog, use `execute_code` — it reaches the entire Blender Python API.

> **Deep dive:** for the full mechanics, every command's parameters, multi-step pipeline patterns, troubleshooting, and worked examples, see the **Blenderer** chapter in `BookOfTlamatini.md`.
...
Installs deps, runs `collectstatic`, executes PyInstaller, copies required payloads (including `README.md`, the self-knowledge map `Tlamatini.md`, and bundled `jd-cli/`), runs migrations, creates the default user (`user`/`changeme`), renames the exe to `Tlamatini.exe`, copies all 78 agent templates, bundles support scripts (`register_flw.ps1`, `CreateShortcut.ps1`, `Tlamatini.ps1`, `Tlamatini.ico`), and zips it all into **`pkg.zip`**.
...
The **external MCP catalog** lives in its own file next to `config.json` — `external_mcps.json` (the `mcpServers` shape, see [§3.21](#321-the-external--mcps-menu--connect-to-any-mcp-server)). You normally manage it from the **External ▸ MCPs** dialog rather than hand-editing it, and `agent/external_mcp_manager.py` reads it in both source and frozen modes.
...
| 5. Unified-agent tools | Synchronous `@tool` functions (core Python + wrapped chat-agent + ACPX/Skill). The **full enabled surface** is bound when Multi-Turn is on (ACPX/Skill tools only when **ACPX** is also ticked). Active only in Multi-Turn. | `agent/tools.py` + `agent/chat_agent_registry.py` + `agent/acpx/` |

> **External MCPs are a separate surface from Layer 2.** Layer 2 is the two built-in *context* MCP daemons (System-Metrics, Files-Search). The **universal external-MCP client** (`agent/external_mcp_manager.py`, catalog in `external_mcps.json`) is distinct: it connects to any declared MCP server over stdio / streamable-HTTP / SSE / WebSocket (up to 5 active), and exposes each server's tools to the Multi-Turn loop as `ext__<server>__<tool>` alongside the Layer-5 tools. See [§3.21](#321-the-external--mcps-menu--connect-to-any-mcp-server).
...
Frontend (toggles) → WebSocket → AgentConsumer → ask_rag() (skips prompt-shape validator)
  → UnifiedAgentChain.invoke() → filter_acpx_tools(tools, acpx_enabled)
    → full enabled tool surface bound (ACPX/Skill tools only when acpx_enabled;
       active external-MCP tools included as ext__<server>__<tool>)
...
### 9.5. Agent catalog (the 78 types, by family)
...
| **Utility** | Parametrizer, FlowBacker, Gatewayer, Gateway-Relayer, Node-Manager, **MCP Doctor** |
