...
<p align="center">
  ...
  A self-hosted AI developer assistant that turns your PC into a workshop where 77 agents collaborate to actually <strong>get things done</strong>.
</p>
...
| 🎨 **Visual workflow designer** | Drag and drop 77 agent types, wire them together, run flows unattended. No code required to compose a pipeline. |
...
| 🔒 **Self-hosted** | The app, hybrid RAG, and all 77 agents run on your machine. The LLM is your choice — fully-local [Ollama](https://ollama.com/) models, Ollama Cloud (the shipped default), or cloud APIs like Anthropic Claude. With a cloud model, your prompts and code context are sent to that provider. |
...
Installs deps, runs `collectstatic`, executes PyInstaller, copies required payloads (including `README.md`, the self-knowledge map `Tlamatini.md`, and bundled `jd-cli/`), runs migrations, creates the default user (`user`/`changeme`), renames the exe to `Tlamatini.exe`, copies all 77 agent templates, bundles support scripts (`register_flw.ps1`, `CreateShortcut.ps1`, `Tlamatini.ps1`, `Tlamatini.ico`), and zips it all into **`pkg.zip`**.
