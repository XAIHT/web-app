...
Download the latest release ZIP — **[Tlamatini v1.48.15](https://github.com/XAIHT/Tlamatini/releases)** — and unzip it (or use a `Tlamatini_Release/` folder somebody handed you / you built — see Part VIII). Then:
...
git tag -a v1.48.15 -m "Release 1.48.15: <one-liner>"   # annotated tag
git push origin v1.48.15
python build.py
python build_uninstaller.py
python build_installer.py
```

All three build scripts pick the tag up from `git describe --tags` automatically. The final artefact lands in `dist/Tlamatini_Release_v1.48.15/`, named for the version so the file you hand to a user is unambiguous before they even unzip it.
...
- **Release folder name** — `dist/Tlamatini_Release_v1.48.15/`.
- **Runtime surfaces** — the About dialog renders `Tlamatini v{{ version }}` (Django context processor); after the release tag/build, the startup banner prints `--- [VERSION] Tlamatini 1.48.15` to both the console and `tlamatini.log`; `GET /agent/version/` returns `{"version":"1.48.15","commit":"abc1234","date":"…","source":"generated"}` as an **open** endpoint suitable for a health-check.
